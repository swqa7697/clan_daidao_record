...
上号@User  [记录上号 | 可@多个用户]
下号@User  [记录下号 | 可@多个用户 | 请在退出实战/登出账号后使用]

我的代刀  [查询自己的代刀状态]
所有代刀  [查询所有人的代刀状态]

清除代刀  [删除自己的代刀状态]
删除代刀@User  [删除指定刀手的所有代刀状态 | 仅@单个用户 | 需要管理权限]
重置代刀  [删除所有代刀状态 | 需要管理权限]

代刀次数(@User): yy-mm-dd: h+d [查询指定用户（不指定则为自己）在指定时间段内的代刀次数 | 可@多个用户 | 例：代刀次数: 22-3-1: 18+4 （3月1日晚上6点到10点 自己的代刀次数）]
...
async def daidaoLogin(bot, ev: CQEvent):
    with status.connect(str(ev.group_id)) as con:
